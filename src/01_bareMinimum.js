...
_.drop = function(arr, n) {
    ...
    if (n <= 0 || n === undefined) {
        ...
        let size = arr.length - n;
        let NewArr2 = [];
        for (let i = 0; i < size; i++) {
            NewArr2.push(arr[size + 1 - i]);
        }
        return NewArr2;
    }
};
...
// _.each는 명시적으로 어떤 값을 리턴하지 않습니다. 즉, 리턴 값이 없다.
_.each = function(collection, iteratee) {
    if (Array.isArray(collection)) { // 배열인 경우 forEach는 쓰면 안 됨
        ...
        // 객체인 경우
        // const obj = {}; 왜 for loop 안에서 obj를 선언해야지만 되지?? 이게 기본 syntax 구나...
        for (const obj in collection) { // obj가 collection 이라는 객체의 key 값이 됨
...
// indexOf( [1,2,3], 4 ); => -1  indexOf( [1,2,3], 3 ); => 2
...
_.filter = function(arr, test) {
    ...
    _.each(arr, function(item) { // item은 접근하는 각각의 엘리먼트 [1,2,3,4,5] = > 1
        if (test(item)) {
...
_.uniq = function(arr) {
    ...
    _.each(arr, function(target) { // target은 접근하는 각각의 엘리먼트 [1,1,2,2,3,4,5] = > 1
        // target은 arr의 각 배열 요소 1, [1] 이 아님!!, Newarr은 빈 배열  
        // indexOf( [1,2,3], 4 ); => -1  indexOf( [1,2,3], 3 ); => 2
        if (_.indexOf(Newarr, target) === -1) {
...
_.map = function(arr, iteratee) {
    ...
    // 이번 iteratee는 리턴 값이 있다. 제곱 함수 이면, iteratee (2) => 4
    let Newarr = [];

    _.each(arr, function(target) {
        Newarr.push(iteratee(target));
...
_.pluck = function(arr, keyOrIdx) {
    ...
    _.map(arr, function(item) {
        ...
        if (!Array.isArray(item)) { // 안이 객체라면
            result.push(item[keyOrIdx]); // push ( value ) => name : gil   ex)  .push( name[gil] )
        } else {
            result.push(item);
        }
...
_.reduce = function(arr, iteratee, initVal) {
    ...
    let result = initVal; // 결과 = 시작값
    // each는 배열의 엘리먼트 안의 갯수만큼 돌아가니까...
    // _.each (arr, function( el ){ ... });  [ 1,2,3]   result => 1
    _.each(arr, function(el, index) { // forEach function 에 두번째 인자는 index 0 , 1, 2 로 사용할 수 있다.

        // arr = [ 1,3,9,10 ] , 함수, initVal = 1

        if (initVal === undefined && index === 0) { // idx는 arr의 index 
            // 시작 값이 정해지지 않았거나, 0번째 index라면,
            // arr의 처음 들어오는 value가 result가 되라
            // reduce ( )는 시작 값을 정해줄 수 있다. 
            result = el;
        } else {
            // 시작 값이 정해져있거나, 정해졌다면, 나머지는 함수가 알아서 하게 하라!!
            result = iteratee(result, el, index, arr);
            // iteratee(acc, ele, idx, arr)
            // 초기값 = 1
            //  el = 1        1,  1, 0, [1,3,9,10]   => result = 2
            //  el = 3        2, 3, 1, [...]  => result = 5
            //  el = 9        5, 9, 2, [,,,] => result = 14
            //  el =10        14, 10, 3, [...] => 24        
        }
    });
    return result;
